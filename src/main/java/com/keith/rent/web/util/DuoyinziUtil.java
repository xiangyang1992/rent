...
//        String str = "曾";
        String string = "zeng";
        List<String> duoyin = getDuoyin(string);
...
//        System.out.println(headP);
    ...
    public static List<String> getDuoyin(String str) {
        ...
        Boolean flag = pinyinMap.containsValue(str);
        Set<Map.Entry<String, List<String>>> entries = pinyinMap.entrySet();
        if (flag) {
            List<String> chinese =  pinyinMap.get(str);
            for (String s : chinese) {
                ...
                for (int i = 0; i < chars.length; i++) {
                    ...
                    if (ch > 128) {
                        try {
                            String[] strings = PinyinHelper.toHanyuPinyinStringArray(ch,defaultFormat);
                            if (strings.length > 1) {
                                logger.debug("多音字{}{}", ch,strings);
                            }
//
    ...
    public static void initPinyin() {
        // 读取多音字的全部拼音表;
//        InputStream file = DuoyinziUtil.class.getResourceAsStream("/duoyinzi_dic.txt");
        InputStream file = DuoyinziUtil.class.getResourceAsStream("/duoyinzi_dic_2.txt");
        ...
        try {
            ...
            while ((s = br.readLine()) != null) {
                if (s != null) {
                    String[] arr = s.split("#");
                    String chinese = arr[0];
                    String[] pinyin = arr[1].split("/");

                    if (chinese != null) {
                        String[] strs = chinese.split(" ");
                        //去空
                        List<String> list = arr2List(strs);
                        pinyinMap.put(chinese, Arrays.asList(pinyin));
                    }
                }
            }
        } catch (IOException e) {
